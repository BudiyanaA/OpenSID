...
#map #collapseStatPenduduk > .card.card-body,
    #map #collapseStatBantuan > .card.card-body {
        max-height: 20vh;
        overflow-y: auto;
    }
        ...
        <div class='modal-content'>
            <div class='modal-header d-flex justify-content-between align-items-center'>
                <h4 class='modal-title' id='myModalLabel'>
                    <i class='fa fa-exclamation-triangle text-red'></i> Judul Modal
                </h4>
                <button type='button' class='close' data-dismiss='modal' aria-hidden='true'>&times;</button>
            </div>
            <div class="fetched-data"></div>
            ...
            const generatePopupDesa = function(data) {
                    ...
                    if (data.pengaturan.includes('Statistik Penduduk')) {
                        ...
                        for (let key in data.list_ref) {
                            _listLink.push(`<li><a href="${_link}/${key}/${data.desa.nama_desa.replace(/\s+/g, '_')}" data-remote="false" data-toggle="modal" data-target="#modalBesar" data-title="Statistik Penduduk ${_title}" >${data.list_ref[key]}</a></li>`)
                        }
                    ...
                    if (data.pengaturan.includes('Statistik Bantuan')) {
                        ...
                        for (let key in data.list_bantuan) {
                            _listLink.push(`<li><a href="${_link}/${key}/${data.desa.nama_desa.replace(/\s+/g, '_')}" data-remote="false" data-toggle="modal" data-target="#modalBesar" data-title="Statistik Bantuan ${_title}">${data.list_bantuan[key]}</a></li>`)
                        }
                ...
                const generatePopupElement = function(data, pengaturan, gis, _link, _title, _wilayah) {
                    ...
                    for (let _key in gis) {
                        ...
                        if (pengaturan.includes('Statistik Penduduk')) {
                            _listLink = []
                            for (let key in data.list_ref) {
                                _listLink.push(`<li><a href="${_link}/${key}/${_params}" data-remote="false" data-toggle="modal" data-target="#modalBesar" data-title="Statistik Penduduk ${_newTitle}" >${data.list_ref[key]}</a></li>`)
                            }
                        ...
                        if (pengaturan.includes('Statistik Bantuan')) {
                            _listLink = []
                            for (let key in data.list_bantuan) {
                                _listLink.push(`<li><a href="${_link}/${key}/${_params}" data-remote="false" data-toggle="modal" data-target="#modalBesar" data-title="Statistik Bantuan ${_newTitle}">${data.list_bantuan[key]}</a></li>`)
                            }
