...
<div id="isi_popup_rt">
    @foreach ($rt_gis as $key_rt => $rt)
        @php
            ...
            $rt_heading = 'Wilayah RT ' . set_ucwords($rt['rt']) . ' RW ' . set_ucwords($rt['rw']) . ' ' . ucwords($this->setting->sebutan_dusun) . ' ' . set_ucwords($rt['dusun']);
        @endphp

        <div id="isi_popup_rt_{{ $key_rt }}" style="visibility: hidden;">
            <div id="content">
                <h5 id="firstHeading" class="firstHeading">{{ $rt_heading }}</h5>
                <div id="bodyContent">
                            ...
                            <div class="card card-body">
                                @foreach ($list_ref as $key => $value)
                                    <li {!! jecho($lap, $key, 'class="active"') !!}>
                                        <a href="{{ site_url("statistik_web/chart_gis_rt/{$key}/{$link}") }}" 
                                           data-remote="false" 
                                           data-toggle="modal" 
                                           data-target="#modalSedang" 
                                           data-title="Statistik Penduduk {{ $data_title }}">
                                           {{ $value }}
                                        </a>
                                    </li>
                                @endforeach
                    ...
                    @if (in_array('Kepala Wilayah', $pengaturan))
                        <p>
                            <a href="{{ site_url("load_aparatur_wilayah/{$rt['id_kepala']}/3") }}" 
                               class="btn btn-social bg-navy btn-sm btn-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" 
                               data-title="Ketua RT {{ underscore($rt['rt']) }}" 
                               data-remote="false" 
                               data-toggle="modal" 
